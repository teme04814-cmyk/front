...
export function ProfessionalStep4({ data, updateData, onNext, onBack }: Step4Props) {
  ...
  const handleFileUpload = (key: string, files: FileList | null) => {
    if (files && files.length > 0) {
      ...
      if (key === "professionalPhoto") {
        updateData({ professional_photo: file, documents: nextDocs })
      } else {
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <input
                    ...
                    type="file"
                    className="hidden"
                    accept={doc.key === 'professionalPhoto' ? 'image/*' : '.pdf,.jpg,.jpeg,.png'}
